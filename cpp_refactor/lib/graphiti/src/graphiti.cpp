...
struct Graphiti::Impl {
    ...
    std::unique_ptr<KuzuGraphStore> store_owned;  // always a KuzuGraphStore (or subclass)
    GraphStore& store;                             // reference to store_owned
    ...
    // Factory: create the right KuzuGraphStore (or KuzuWriterGraphStore subclass)
    static std::unique_ptr<KuzuGraphStore> make_store(const GraphitiConfig& config) {
        bool read_only = config.read_only || !config.kuzu_writer_uri.empty();
        if (!config.kuzu_writer_uri.empty()) {
            auto ws = std::make_unique<KuzuWriterGraphStore>(
                config.db_path, config.kuzu_writer_uri, config.kuzu_writer_target_db);
            auto r = ws->connect();
            if (r.has_value()) {
        ...
        return std::make_unique<KuzuGraphStore>(config.db_path, read_only);
    }

    static std::unique_ptr<KuzuGraphStore> make_store(kuzu::main::Database& shared_db) {
        return std::make_unique<KuzuGraphStore>(shared_db);
    }

    // Default: create OpenAI clients from config
    Impl(GraphitiConfig cfg)
        : config(std::move(cfg))
        , store_owned(make_store(config))
        , store(*store_owned)
        , llm_owned(std::make_unique<OpenAIClient>(config.llm))
        , embedder_owned(std::make_unique<OpenAIEmbedder>(config.embedder))
        , llm(llm_owned.get())
        , embedder(embedder_owned.get()) {}

    // Shared DB + default OpenAI clients
    Impl(GraphitiConfig cfg, kuzu::main::Database& shared_db)
        : config(std::move(cfg))
        , store_owned(make_store(shared_db))
        , store(*store_owned)
        , llm_owned(std::make_unique<OpenAIClient>(config.llm))
        , embedder_owned(std::make_unique<OpenAIEmbedder>(config.embedder))
        , llm(llm_owned.get())
...
RETURN DISTINCT b.uuid AS uuid)", gid);
    ...
    auto* db = impl_->store_owned->database();
...
Result<AddEpisodeResult> Graphiti::add_episode(AddEpisodeOptions opts) {
    ...
    auto save_ep = impl_->store.persist_episode(episode);
...
{{"edges": [{{"source_entity_name": "...", "target_entity_name": "...", "relation_type": "...", "fact": "...", "valid_at": null, "invalid_at": null}}]}})",
    ...
    for (auto& node : nodes) {
        ...
        if (is_existing) {
            ...
            auto existing = impl_->store.get_entity(node.uuid);
    ...
    if (opts.saga.has_value() && !opts.saga.value().empty()) {
        ...
        auto existing = impl_->store.find_saga(opts.saga.value(), gid);
        SagaNode saga_node;
        ...
        if (opts.saga_previous_episode_uuid.has_value() && !opts.saga_previous_episode_uuid.value().empty()) {
            ...
            auto last = impl_->store.get_last_saga_episode(saga_node.uuid, episode.uuid);
...
Result<AddBulkEpisodeResults> Graphiti::add_episode_bulk(AddEpisodeBulkOptions opts) {
    ...
    for (auto& ep : episodes) {
        ...
        auto r = impl_->store.persist_episode(ep);
    ...
    if (opts.saga.has_value() && !opts.saga.value().empty()) {
        ...
        auto last = impl_->store.get_last_saga_episode(saga_node.uuid);
        if (last.has_value() && last.value().has_value()) {
            prev_ep_uuid = last.value().value();
        }
...
kuzu::main::Database& Graphiti::database() const {
    return *impl_->store_owned->database();
}
