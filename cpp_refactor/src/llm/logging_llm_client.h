...
// Logs request BEFORE sending and response AFTER receiving, for each retry attempt.
...
public:
    ...
    Result<nlohmann::json> generate_response(
        ...
        // Set up per-attempt callback so inner's retry loop logs every attempt
        ...
        inner_->on_attempt = [&](const std::vector<Message>& msgs,
            ...
            info.attempt = is_pre_call ? 0 : attempt_counter;
